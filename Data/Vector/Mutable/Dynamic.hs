...
-- |Convert a mutable vector to an immutable one without copying. The mutable
-- vector shouldn't be accessed afterwards.
...
-- |Create a new vector of given length. The elements are uninitialized and
-- throw error upon accessing. The "Int" argument must be positive.
...
-- |Returns a vector consisting of a value repeated the given times. Throws an
-- error if the "Int" argument is negative.
...
-- |Move the contents of the right vector to the left one. Inputs must have the
...
-- |Move the contents of the right vector to the left one. The vectors must be
-- the same length but may overlap.
...
-- |Move the contents of the right vector to the left one. The vectors must
-- have the same length and may overlap. Input lengths are unchecked.
...
-- |Ensure that an amount of capacity is reserved in the vector. A no-op if
-- there is already enough capacity. Throws an error if the argument is
-- negative.
...
-- |Ensure that an amount of capacity is reserved in the vector. A no-op if
-- there is already enough capacity. The argument is unchecked.
...
-- |Increment the size of the vector and write a value to the back. Pushing to
-- a slice will potentially overwrite the original vector's elements.
...
-- |Read the back value and remove it from the vector. Throws an error if the
-- vector is empty.
...
-- |Apply a function to the vector recast as immutable. This is usually unsafe
-- if we later modify the vector.
